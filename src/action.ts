...
export default async function run() {
  ...
  while (waiting) {
    ...
    if (latestStage.status === "failed" || latestStage.status === "failure") {
      ...
      if (slackWebHook) {
        const logs = await getCloudflareLogs(
          authHeaders,
          accountId,
          project,
          deployment.id
    ...
    async function getCloudflareLogs(
      ...
      try {
        ...
        if (!res.ok) {
          console.error(
            `Failed to fetch Cloudflare logs - Status code: ${res.status} (${res.statusText})`
          );
          return "";
        }
    ...
    if (
      ...
      if (deployment.latest_stage.status === "success" && true) {
        slack
          .send(
            `:white_check_mark: CloudFlare Pages \`Deployment\` pipeline for project *${project}* \`SUCCEEDED\`!\nEnvironment: *${deployment.environment}\nActor: *${actor}*\nAlias URL: ${aliasUrl}\nBuild includes: *${deployment.deployment_trigger.metadata.commit_message}*`
          )
          .then(() => {
            console.log(
              "Slack message for DEPLOYMENT succeeded pipeline sent!"
            );
          })
          .catch((err) => {
            console.error(err);
